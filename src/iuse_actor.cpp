...
std::optional<int> link_up_actor::use( Character *p, item &it, const tripoint &pnt ) const
{
    ...
    if( !is_cable_item || !it.link || it.link->has_no_links() ) {
        ...
        link_menu.help_text = it.link && it.link->t_veh_safe ? string_format( _( "Attached to: %s" ), it.link->t_veh_safe->name ) : "";
        if( targets.count( link_state::vehicle_port ) > 0 ) {
        ...
        link_menu.set_title( string_format( _( "What to do with the %s?" ), cable_name ) );
        link_menu.help_text = it.link->t_veh_safe ? string_format( _( "Attached to: %s" ), it.link->t_veh_safe->name ) : "";
        ...
        link_menu.set_title( string_format( _( "What to do with the %s?%s%s" ), cable_name,
